...
sink(paste0(output_folder,gene_name,"_log_glasso_",date_time,".txt"))
...
##3: subset the tissues metadata
metadata_celltype <- subset(metadata, tissue==opt$tissue) 

##4: process and run glasso on all genes in chromosome
glasso_pairs <- process_glasso(gene_name=gene_name, 
